...
3. 选择子集化后端，按需勾选各选项：「居中思源省略号」「调试选项」「内嵌字体」（内嵌进输出字幕）「字体放入同名文件夹」（**默认开**，字体放入与 ass 同名的子文件夹）「仅内嵌」（内嵌后不再输出独立子集字体文件，只留自带字体的 ass）「重新内嵌」（对已内嵌字体的 ass：以其内嵌字体为源，删除旧内嵌并重新子集化内嵌，无需外部字体）。
