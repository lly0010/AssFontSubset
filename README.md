...
2. 添加需要子集化的 ASS 字幕文件：可直接拖入文件或文件夹、点「浏览」选择文件、或点「添加文件夹」批量导入；列表中可多选后用「删除所选」或 Delete 键单独移除。「字体目录」「输出目录」会自动填入第一个字幕同目录下的 `fonts`、`output`，也可自行修改或拖入文件夹设置。
...
6. 「字体数据库」一栏可指定索引 JSON 路径，点「构建数据库」会扫描「字体目录」并生成该索引（便于查找字体）。
