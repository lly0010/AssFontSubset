...
Options:
  ...
  --separate-font-folder                         将每个子集化字体放入单独的子文件夹，文件夹与字体同名 [default: False]
```

> 独立字体文件夹（`--separate-font-folder`）会把每个子集字体移动到与其同名的子文件夹中，即 `output/<字体名>/<字体名>.ttf`；输出的字幕文件仍位于输出目录顶层。
...
3. 选择子集化后端，按需勾选「居中思源省略号」「调试选项」「内嵌字体」（内嵌进输出字幕）「独立字体文件夹」（每个字体放入同名子文件夹）。
