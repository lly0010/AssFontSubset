...
> `--reembed-fonts`：针对**已内嵌字体的 ASS**。直接从其 `[Fonts]` 段解码取出内嵌字体作为字体源，删除原有内嵌,重新子集化后再内嵌一次。多个文件会**各自独立处理**。若某个文件**没有内嵌字体**，则自动回退到用「字体目录 / 字体数据库」正常子集化（仍会内嵌），因此可以把"已内嵌"和"未内嵌"的字幕混在一批里一起处理。
>
